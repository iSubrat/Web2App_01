...
def execute_query(db_host, db_username, db_password, db_database, query):
    ...
    try:
        ...
        if row:
            ...
            update_query = "UPDATE app_data SET status = 'BUILDING_01', status_updated_at = NOW() WHERE id = %s"
            cursor.execute(update_query, (id,))
            connection.commit()
            print("Status column updated to 'BUILDING_01'")
        else:
...
if __name__ == "__main__":
    try:
      ...
      # Example query
      query = "SELECT * FROM app_data WHERE status = 'PENDING_01' ORDER BY id DESC LIMIT 1"
